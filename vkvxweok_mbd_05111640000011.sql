...
/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getuserid`(IN `p_username` VARCHAR(15))
BEGIN
...
/* Procedure structure for procedure `sp_listcourse` */

/*!50003 DROP PROCEDURE IF EXISTS  `sp_listcourse` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listcourse`()
BEGIN
		SELECT `COURSE_ID`, `SKILL_NAME`, `COURSE_DESCRIPTION`, `COURSE_STAMINA`, `COURSE_DURATION`, `COURSE_COST`, `COURSE_EXPERIENCE` FROM `course` JOIN `skill` ON `course`.`SKILL_ID` = `skill`.`SKILL_ID`;
	END */$$
DELIMITER ;

/* Procedure structure for procedure `sp_listequip` */

/*!50003 DROP PROCEDURE IF EXISTS  `sp_listequip` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listequip`(`p_userid` INT(11))
BEGIN
		...
		UNION
		select `equipment`.`UPGRADE_ID`, `USER_ID`,`UPGRADE_NAME`, `USER_EQUIPMENT_LEVEL`, `UPGRADE_COST`, `UPGRADE_LEVELCOST` from `equipment` JOIN `user_equipment` ON `equipment`.`UPGRADE_ID` = `user_equipment`.`UPGRADE_ID` AND `user_equipment`.`USER_ID`= p_userid
		ORDER BY USER_ID DESC;
	END */$$
DELIMITER ;

/* Procedure structure for procedure `sp_listjob` */

/*!50003 DROP PROCEDURE IF EXISTS  `sp_listjob` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listjob`()
BEGIN
		SELECT `JOB_ID`, `JOB_NAME`, `JOB_DESCRIPTION`, `JOB_EXPERIENCE`, `JOB_PAYMENT`, `JOB_STAMINA`, `JOB_DURATION` FROM `job`;
		select `job`.`JOB_NAME`, group_concat(`SKILL_NAME` separator ', ') as 'REQUIREMENT', group_concat(`equipment`.`UPGRADE_NAME` separator ',') as 'EQ_NAME' , group_concat(`JOB_EQUIPMENT_LEVEL` separator ',') as 'EQ_LV'
